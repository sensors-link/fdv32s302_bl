...
/**
  ...
  * @retval None
  */
void IOM_SyncAPBCmd(u32 IOM_Pin, FunctionalState NewState)
{
	/* Check the parameters */
...
/**
  ...
  * @retval None
  */
void IOM_DebounceCmd(u32 IOM_Pin, FunctionalState NewState)
{
	/* Check the parameters */
	PARAM_CHECK(IS_FUNCTIONAL_STATE(NewState));
...
{
	/* Return the data in the DATAIN register */
	return IOM->DATAIN;
}
...
void IOM_ClrPin(u32 IOM_Pin)
{
	...
	/* Clear selected bits in DATAOUT */
	IOM->DATAOUT &= ~IOM_Pin;
}
