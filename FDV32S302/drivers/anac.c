...
#define ADC_CFG_CLEAR_Mask ((u32)0xFFFFF808)
...
/**
  * @brief  Configure LED output positive and negative temperature coefficient.
  * @param  TempGear: Specify temperature compensation gear.
  *   This parameter can be one of the following values:
  *     @arg TEMP_GEAR_SEL_LOW: Temperature coefficient compensation low value.
  *     @arg TEMP_GEAR_SEL_HIGH: Temperature coefficient compensation high value.
  * @param  TempCoefficient: Specify the positive and negative temperature coefficient of LED output.
...
{
	/* Check the parameters */
	PARAM_CHECK(IS_LED_TEMPGEAR(TempGear));
	PARAM_CHECK(IS_LED_TEMPCOEFFICIENT(TempCoefficient));

	/* Set temperature coefficient compensation gear */
	if(TempGear == TEMP_GEAR_SEL_LOW)
	{
		ANAC->LED_CFG &= ~ANAC_LED_CFG_LED_TEMP_SEL;
	}
	else
	{
		ANAC->LED_CFG |= ANAC_LED_CFG_LED_TEMP_SEL;
	...
	/* Set LED output positive and negative temperature coefficient */
	ANAC->LED_CFG &= ~ANAC_LED_CFG_LEDOPT;
	ANAC->LED_CFG |= TempCoefficient << ANAC_LED_CFG_LEDOPT_pos;
}

/**
  * @brief  Configure the LED drive current according to the incoming parameters.
  * @param  LED_Sel: Specify the selection of infrared tube.
  *   This parameter can be one of the following values:
  *     @arg LED_SEL_IIRLED: Infrared red light tube
  *     @arg LED_SEL_IBULE: Infrared blue light tube
  * @param  CurrentGear: Specify the current gear of the selected infrared tube.
  *   This parameter can be one of the following values:
  *     @arg LED_GEAR_SEL_15MA: Select 15mA as a step for LED current gear.
  *     @arg LED_GEAR_SEL_7P8MA: Select 7.8mA as a step for LED current gear.
  * @param  CurrentVal: Specify the current value of the selected infrared tube.
  *   This parameter value must be lower than 0x20.
  * @retval None
  */
void LED_DriveCurrentConfig(u8 LED_Sel, u8 CurrentGear, u8 CurrentVal)
{
	...
	PARAM_CHECK(IS_LED_CURRENTGEAR(CurrentGear));
	PARAM_CHECK(IS_LED_CURRENTVAL(CurrentVal));

	if (LED_Sel == LED_SEL_IIRLED)
	{
		/* Set the current gear of infrared red light tube */
		if(CurrentGear == LED_GEAR_SEL_15MA)
		{
			ANAC->LED_CFG &= ~ANAC_LED_CFG_IRLEDRANGE_SEL;
		}
		else
		{
			ANAC->LED_CFG |= ANAC_LED_CFG_IRLEDRANGE_SEL;
	...
	{
		/* Set the current gear of infrared blue light tube */
		if(CurrentGear == LED_GEAR_SEL_15MA)
		{
			ANAC->LED_CFG &= ~ANAC_LED_CFG_BLEDRANGE_SEL;
		}
		else
		{
			ANAC->LED_CFG |= ANAC_LED_CFG_BLEDRANGE_SEL;
...
/**
  * @brief  Enable or disable the LVD trigger chip reset.
  * @param  NewState: New state of LVD trigger chip reset.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void LVD_TriggerResetCmd(FunctionalState NewState)
{
	...
	if (NewState == ENABLE)
	{
		/* Enable LVD trigger chip reset */
		ANAC->LVD_CFG |= ANAC_LVD_CFG_ACT_SEL;
	}
	else
	{
		/* Disable LVD trigger chip reset */
		ANAC->LVD_CFG &= ~ANAC_LVD_CFG_ACT_SEL;
	}
}
...
/**
  ...
  *     @arg AINSEL_MULTIPLEX_P15_SEL_ATS0 : GP15 multiplex select ATS0
  ...
  *     @arg AINSEL_MULTIPLEX_P17_SEL_VC1N2: GP17 multiplex select VC1N2
...
/**
  * @brief  Configure the time from PD reset to LED enable.
  * @param  PDRst2LEDDelayTime: Specify the time from PD reset to LED enable.
  *   This parameter can be one of the following values:
  *     @arg DLY_PD_RST2LED_DLY_10: Settling time required is 10 anac_500k_clk clock cycles
  *     @arg DLY_PD_RST2LED_DLY_20: Settling time required is 20 anac_500k_clk clock cycles
  *     @arg DLY_PD_RST2LED_DLY_50: Settling time required is 50 anac_500k_clk clock cycles
  *     @arg DLY_PD_RST2LED_DLY_500: Settling time required is 500 anac_500k_clk clock cycles
  * @retval None
  */
void DLY_PDRst2LEDDelayConfig(u8 PDRst2LEDDelayTime)
{
	/* Check the parameters */
	PARAM_CHECK(IS_DLY_PDRST2LEDDLY(PDRst2LEDDelayTime));

	/* Set the time from PD reset to LED enable */
	ANAC->DLY_CFG &= ~ANAC_DLY_CFG_LED_STA_DLY;
	ANAC->DLY_CFG |= PDRst2LEDDelayTime << ANAC_DLY_CFG_LED_STA_DLY_pos;
}
